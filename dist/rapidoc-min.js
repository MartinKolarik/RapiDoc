...
`;customElements.define("schema-tree",class extends ie{static get properties(){return{data:{type:Object},schemaExpandLevel:{type:Number,attribute:"schema-expand-level"},schemaDescriptionExpanded:{type:String,attribute:"schema-description-expanded"},allowSchemaDescriptionExpandToggle:{type:String,attribute:"allow-schema-description-expand-toggle"},schemaHideReadOnly:{type:String,attribute:"schema-hide-read-only"},schemaHideWriteOnly:{type:String,attribute:"schema-hide-write-only"}}}connectedCallback(){super.connectedCallback(),(!this.schemaExpandLevel||this.schemaExpandLevel<1)&&(this.schemaExpandLevel=99999),this.schemaDescriptionExpanded&&"true false".includes(this.schemaDescriptionExpanded)||(this.schemaDescriptionExpanded="false"),this.schemaHideReadOnly&&"true false".includes(this.schemaHideReadOnly)||(this.schemaHideReadOnly="true"),this.schemaHideWriteOnly&&"true false".includes(this.schemaHideWriteOnly)||(this.schemaHideWriteOnly="true")}static get styles(){return[Ke,zP,SP,c`
    ...
    `}generateTree(e,t="object",r="",n="",o="",a=0,i=0,s=""){var l;if("true"===this.schemaHideReadOnly){if("array"===t&&"readonly"===s)return;if("readonly"===(null==e?void 0:e["::readwrite"]))return}if("true"===this.schemaHideWriteOnly){if("array"===t&&"writeonly"===s)return;if("writeonly"===(null==e?void 0:e["::readwrite"]))return}if(!e)return q`<div class="null" style="display:inline;">
      ...
      </div>`;if(0===Object.keys(e).length)return q`<span class="key object">${n}:{ }</span>`;let c="",p="";if(n.startsWith("::ONE~OF")||n.startsWith("::ANY~OF"))c=n.replace("::","").replace("~"," ");else if(n.startsWith("::OPTION")){const e=n.split("~");[,c,p]=e}else c=n;const d=400-12*i;let u="",h="";const f=null!==(l=e["::type"])&&void 0!==l&&l.startsWith("xxx-of")?a:a+1,m="xxx-of-option"===t||"xxx-of-option"===e["::type"]||n.startsWith("::OPTION")?i:i+1,y=Object.keys(e).some((e=>e.startsWith("::ANY~OF")||e.startsWith("::ONE~OF"))),g=a;if(a=y?0:a,"object"===e["::type"])"array"===t?(u=a<this.schemaExpandLevel?y?q`<span class="open-bracket array-of-object" >[</span>`:q`<span class="open-bracket array-of-object" >[{</span>`:q`<span class="open-bracket array-of-object">[{...}]</span>`,h=y?"]":"}]"):(u=a<this.schemaExpandLevel?y?q`<span class="open-bracket object">${e["::nullable"]?"null┃":""}</span>`:q`<span class="open-bracket object">${e["::nullable"]?"null┃":""}{</span>`:q`<span class="open-bracket object">${e["::nullable"]?"null┃":""}{...}</span>`,h=y?"":"}");else if("array"===e["::type"])if("array"===t){const e="object"!==r?r:"";u=a<this.schemaExpandLevel?y?q`<span class="open-bracket array-of-array" data-array-type="${e}">[ ${e} </span>`:q`<span class="open-bracket array-of-array" data-array-type="${e}">[[ ${e} </span>`:q`<span class="open-bracket array-of-array"  data-array-type="${e}">[[...]]</span>`,h=y?"]":"]]"}else u=a<this.schemaExpandLevel?y?q`<span class="open-bracket array"></span>`:q`<span class="open-bracket array">[</span>`:q`<span class="open-bracket array">[...]</span>`,h=y?"":"]";var v;if("object"==typeof e)return q`
        <div class="tr ${a<this.schemaExpandLevel||null!==(v=e["::type"])&&void 0!==v&&v.startsWith("xxx-of")?"expanded":"collapsed"} ${e["::type"]||"no-type-info"}${e["::nullable"]?" nullable":""}" title="${e["::deprecated"]?"Deprecated":""}">
          <div class="td key ${e["::deprecated"]?"deprecated":""}" style='min-width:${d}px'>
            ${"xxx-of-option"===e["::type"]||"xxx-of-array"===e["::type"]||n.startsWith("::OPTION")?q`<span class='key-label xxx-of-key'> ${c}</span><span class="xxx-of-descr">${p}</span>`:"::props"===c||"::ARRAY~OF"===c?"":g>0?q`<span class="key-label" title="${"readonly"===s?"Read-Only":"writeonly"===s?"Write-Only":""}">
                      ${e["::deprecated"]?"✗":""}
      ...
      `;const[b,x,w,$,k,S,A,E,O]=e.split("~|~");if("🆁"===x&&"true"===this.schemaHideReadOnly)return;if("🆆"===x&&"true"===this.schemaHideWriteOnly)return;const T=b.replace(/┃.*/g,"").replace(/[^a-zA-Z0-9+]/g,"").substring(0,4).toLowerCase(),C=""+(w||$||k||S?`<span class="descr-expand-toggle ${"true"===this.schemaDescriptionExpanded?"expanded-descr":""}">➔</span>`:"");let j="",I="";return"array"===t?"readonly"===s?(j="🆁",I="Read-Only"):"writeonly"===s&&(j="🆆",I="Write-Only"):"🆁"===x?(j="🆁",I="Read-Only"):"🆆"===x&&(j="🆆",I="Write-Only"),q`
      <div class = "tr primitive" title="${O?"Deprecated":""}">
        <div class="td key ${O}" style='min-width:${d}px'>
          ...
          <span class="${T}" title="${I}"> 
            ${"array"===t?`[${b}]`:`${b}`}
            ${j}
          </span>
        </div>
        <div class='td key-descr'>
          ${o||E||A?q`${q`<span class="m-markdown-small">
                ${Q_(We("array"===t?`${C} ${o}`:E?`${C} <b>${E}:</b> ${A}`:`${C} ${A}`))}
              </span>`}`:""}  
          ${w?q`<div style='display:inline-block; line-break:anywhere; margin-right:8px'><span class='bold-text'>Constraints: </span>${w}</div>`:""}
    ...
    `}formDataParamAsObjectTemplate(e,t,r){var n;const o=DP(t,{}),a=NP(t,"json",t.examples,t.example,"true"===this.callback||"true"===this.webhook,"true"!==this.callback&&"true"!==this.webhook,"text",!1);return q`
...
`}
    ...
    `}resetSelection(){this.selectedStatus="",this.selectedMimeType=""}responseTemplate(){if(!this.responses)return"";for(const n in this.responses){this.selectedStatus||(this.selectedStatus=n);const o={};for(const r in null===(e=this.responses[n])||void 0===e?void 0:e.content){var e,t;const a=this.responses[n].content[r];this.selectedMimeType||(this.selectedMimeType=r);const i=DP(a.schema,{}),s=NP(a.schema,r,a.examples,a.example,"true"!==this.callback&&"true"!==this.webhook,"true"===this.callback||"true"===this.webhook,r.includes("json")?"json":"text");o[r]={description:this.responses[n].description,examples:s,selectedExample:(null===(t=s[0])||void 0===t?void 0:t.exampleId)||"",schemaTree:i}}const a=[];for(const e in null===(r=this.responses[n])||void 0===r?void 0:r.headers){var r;a.push({name:e,...this.responses[n].headers[e]})}this.headersForEachRespStatus[n]=a,this.mimeResponsesForEachStatus[n]=o}return q`
